...
speed = Token:match("(%d+)")
redis:set(speed..":VERSION",1)
redis:set(speed..":SUDO_ID:",SUDO_USER)
redis:set(speed..":DataCenter:",'German')
redis:set(speed..":UserNameBot:",BOT_User)
redis:set(speed..":NameBot:",BOT_NAME)
redis:hset(speed..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3speed_INSTALL","Yes")
info = {}
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس اسبيد بنسبه 100% ]  }•\e[0m"
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @Source_speed }• }•\e[0m"
...
echo -e "\e[38;5;77m      SPEED ✾ @V_P_Ee[0m"
...
speed = Token:match("(%d+)")
our_id = tonumber(speed)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(speed..":UserNameBot:")
SUDO_ID = tonumber(redis:get(speed..":SUDO_ID:"))
SUDO_USER = redis:hgetall(speed..'username:'..SUDO_ID).username
version = redis:get(speed..":VERSION")
DataCenter = redis:get(speed..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..speed..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(speed..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(speed..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(speed..'group:add'..msg.chat_id_) then 
...
msg.TheRank =  'مطور السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1463818802 then 
...
msg.Rank = 1
elseif redis:sismember(speed..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(speed..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(speed..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(speed..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(speed..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(speed..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(speed..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(speed..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(speed..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(speed..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(speed..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(speed..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس اسبيد ⇐ @Source_speed
...
if msg.GroupActive and redis:get(speed..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(speed..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(speed..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(speed..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.speed then 
if msg.text and ScriptFile.ispeed then
for k, speed in pairs(ScriptFile.speed) do
local SearchText = msg.text:match(speed)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ispeed(msg,{msg.text:match(speed)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",speed.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dspeed then
if ScriptFile.dspeed(msg) == false then
return false
end
...
if Plug.speed then 
if msg.text and Plug.ispeed then
for k, speed in pairs(Plug.speed) do
local SearchText = msg.text:match(speed)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(speed..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(speed..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ispeed(msg,{msg.text:match(speed)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",speed.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source speed")
end 

end
else
print("The File Script.lua Not Runing in The Source speed")
...
function tdcli_update_callback(data)
	...
	redis:srem(speed..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(speed..":SUDO_ID:")
redis:hset(speed..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(speed..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(speed..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(speed..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(speed..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(speed..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(speed..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(speed..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(speed..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(speed..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(speed..'group:add'..data.chat_.id_) then
	redis:set(speed..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(speed..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(speed..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(speed..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	speed..'group:name'..k,IDS.Title,
	speed..'num_msg_speed'..k,5,
	speed..'group:add'..k,true,
	speed..'lock_link'..k,true,
	speed..'lock_spam'..k,true,
	speed..'lock_webpage'..k,true,
	speed..'lock_markdown'..k,true,
	speed..'lock_flood'..k,true,
	speed..'lock_bots'..k,true,
	speed..'mute_forward'..k,true,
	speed..'mute_contact'..k,true,
	speed..'mute_document'..k,true,
	speed..'mute_inline'..k,true,
	speed..'lock_username'..k,true,
	speed..'replay'..k,true
	)
	redis:sadd(speed..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(speed..'username:'..ID,'username',user)
	redis:sadd(speed..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(speed..'username:'..ID,'username',user)
	redis:sadd(speed..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(speed..'username:'..ID,'username',user)
	redis:sadd(speed..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(speed..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(speed..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(speed..'username:'..data.user_.id_).username
	...
	redis:hset(speed..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
