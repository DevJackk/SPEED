...
local Text = "♡‿♡ أهلاً [المطور الاساسي](tg://user?id="..SUDO_USER..") \n♡‿♡ شكراً لأستخدام سورس اسبيد \n♡‿♡ أرسل /start\n♡‿♡ لأظهار الاوامر المطور  المجهزه بالكيبورد\n\n."
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       ♡‿♡{ تنصيب سورس اسبيد بنسبه 100% ]  }♡‿♡\e[0m"
echo -e "\e[38;5;77m       ♡‿♡{ ♡‿♡{ Join the Source Channel : @Source_speed }♡‿♡ }♡‿♡\e[0m"
...
echo -e "\e[38;5;77m       ♡‿♡{ BD source has been installed }♡‿♡\e[0m"
echo -e "\e[38;5;77m       ♡‿♡{ developer JACK : @V_P_E  }♡‿♡\e[0m"
...
SUDO_USER = redis:hgetall(black..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(black..':WELCOME_BOT'),[[♡‿♡ مرحباً انا بوت ]]..redis:get(black..':NameBot:')..[[ .
♡‿♡ اختـصاصـي حمايـة الـمجموعات
♡‿♡ تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
♡‿♡ قنـاة سورس اسبيد ⇐ @Source_speed
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	return sendMsg(msg.chat_id_,msg.id_," ♡‿♡ تم تحديث الملفات .\n",nil,function(arg,data)
	Refresh_Start = true
	end)
	end 
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/SPEED/main/plugins/games.lua','./plugins/games.lua')
	sendMsg(msg.chat_id_,msg.id_,'♡‿♡ {* تــم تحديث وتثبيت السورس  *} .\n\n♡‿♡ { Bot is Update » }',nil,function(arg,data)
	dofile("./inc/Run.lua")
...
redis:hset(black..'username:'..tonumber(mmd),'username',msg.text)
	send_msg(msg.chat_id_,"♡‿♡ عزيزي تم تغيير المطور الاساسي بنجاح الان ارسل reload ...")
	end
...
send_msg(msg.chat_id_,"♡‿♡ تم تثبيت الايدي الان قم برسال معرف المطور @UserName ...")
	end
	if msg.text== 'تغير المطور الاساسي' and msg.sender_user_id_ == SUDO_ID then
    send_msg(msg.chat_id_,"♡‿♡ عزيزي قم برسال ايدي المطور ...")
redis:setex('setid'..msg.sender_user_id_,120,true)
end
	
	
	if msg.text== 'reload' and msg.sender_user_id_ == SUDO_ID then
	sendMsg(msg.chat_id_,msg.id_,'♡‿♡ {* تــم أعـاده تشغيل البوت  *} .\n\n♡‿♡ { Bot is Reloaded » }.',nil,function(arg,data)
	dofile("./inc/Run.lua")
	print("Reload ~ ./inc/Run.lua")
	end)
	return false
...
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	send_msg(SUDO_ID,"♡‿♡ قام شخص بطرد البوت من المجموعه الاتيه : \n♡‿♡ ألايدي : `-100"..data.channel_.id_.."`\n♡‿♡ الـمجموعه : "..Flter_Markdown(NameGroup).."\n\n♡‿♡ تـم مسح كل بيانات المجموعه بنـجاح ")
	...
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'♡‿♡ ** تم رفع النسخه‏‏ الاحتياطيه\n♡‿♡ ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(black..'group:ids')..' .*\n')
	end
