...
local message = '*♡‿♡ المنشئيين :*\n\n'
local monsha = redis:smembers(speed..':MONSHA_BOT:'..msg.chat_id_)
if #monsha == 0 then 
message = message .."♡‿♡ Not Creator ~⪼ لا يوجد منشئيين .\n"
else
...
if #list == 0 then  
message = message.."♡‿♡ Not Director < لا يوجد مدراء .\n" 
else
for k,v in pairs(list) do
local info = redis:hgetall(speed..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
...
if #list==0 then  return  "♡‿♡ ** لا يوجد ادمن في هذه المجموعه \n" end
message = '♡‿♡ * قائمه الادمنيه :*\n\n'
for k,v in pairs(list) do
local info = redis:hgetall(speed..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' » ❪`' ..v.. '`❫ \n'
...
if #list == 0 then return "*♡‿♡ لا يوجد مميزين في القائمه *" end
message = '♡‿♡ ** قائمه الاعضاء المميزين :\n'   
for k,v in pairs(list) do
local info = redis:hgetall(speed..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' » ❪`' ..v.. '`❫ \n'
...
if #list==0 then return "♡‿♡ **لايوجد اعضاء مكتومين " end
message = '♡‿♡ **قائمه الاعضاء المكتومين :\n'
for k,v in pairs(list) do
local info = redis:hgetall(speed..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' » ❪`' ..v.. '`❫ \n'
...
if #list==0 then return "♡‿♡ **لايوجد أعضاء محظورين " end
...
if #list==0 then return  "*♡‿♡ لايوجد اعضاء محظورين عام*" end
message = '♡‿♡ **قائمه المحظورين عام :\n'
for k,v in pairs(list) do
local info = redis:hgetall(speed..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' » ❪`' ..v.. '`❫ \n'
...
local list = redis:smembers(speed..':Filter_Word:'..msg.chat_id_)
if #list == 0 then return "♡‿♡ **قائمه الكلمات الممنوعه فارغه" end
filterlist = '♡‿♡ **قائمه الكلمات الممنوعه :\n'    
for k,v in pairs(list) do
...
return  "♡‿♡ **الكلمه *{"..word.."}* هي بالتأكيد من قائمه المنع ." 
else
redis:sadd(speed..':Filter_Word:'..msg.chat_id_,word) 
return  "♡‿♡ **الكلمه *{"..word.."}* تمت اضافتها الى قائمه المنع ."
end
end
...
return  "♡‿♡ **الكلمه *{"..word.."}* تم السماح بها .️" 
else
return  "♡‿♡ **الكلمه *{"..word.."}* هي بالتأكيد مسموح بها ." 
end
end
...
Save_Data:close()
sendDocument(msg.chat_id_,msg.id_,"./inc/"..Bot_User..".json","♡‿♡ ملف النسخه الاحتياطيه ...\n♡‿♡ المجموعات » { "..#All_Groups_ID.." }\n♡‿♡ للبوت » "..Bot_User.."\n♡‿♡ التاريخ » "..os.date("%Y/%m/%d").."\n",dl_cb,nil)
end

function chat_list(msg)
local list = redis:smembers(speed..'group:ids')
message = '♡‿♡ **قائمه المجموعات :\n\n'
for k,v in pairs(list) do 
...
message = message..k.. 'ـ '..' ☜ ♡‿♡⊱ { `' ..v.. '` } ⊰♡‿♡ \n'
end 
end
all_groups = '♡‿♡ قائمه المجموعات :<br><br>'
for k,v in pairs(list) do 
local info = redis:get(speed..'group:name'..v)
if info then
...
sendMsg(msg.chat_id_,1,'♡‿♡ **عذرا لديك الكثير من المجموعات\n♡‿♡ **سوف ارسل لك ملف فيها قائمه مجموعات المفعله انتظر لحظه ...')
...
<style>*{font-family: 'Harmattan', sans-serif;font-weight: 600;text-shadow: 1px 1px 16px speed;}</style>
<body>
...
file:close()
return sendDocument(msg.chat_id_,msg.id_,'./inc/All_Groups.html','♡‿♡ قائمه المجموعات بالكامله ✓ \n♡‿♡ يحتوي ('..#list..') مجموعه \n♡‿♡ افتح الملف في عارض HTML او بالمتصفح',dl_cb,nil)
else 
...
if NumAdmin == 0 then 
return sendMsg(msg.chat_id_,msg.id_,"♡‿♡ لا يـوجـد أدمـنـيـه لكِ يتـم رفعهم \n")
else
return sendMsg(msg.chat_id_,msg.id_,"♡‿♡ تم رفع  { *"..NumAdmin.."* } مـن الادمنيه‌‏ في البوت \n")
end
end,100)
end
...
if not msg.SudoUser and not service then return '♡‿♡ أنـت لـسـت الـمـطـور .' end
if msg.is_post_ then return "♡‿♡ عذرا هذا بوت حمايه المجموعات وليس القنوات  " end
if msg.type ~= "channel" then return '♡‿♡ لا يمكنك تفعيل البوت في المجوعات العاديه / البوت يدعم فقط المجموعات الخارقه .' end
if redis:get(speed..'group:add'..msg.chat_id_) then  return '♡‿♡ **المجموعة تم تفعيلها مُسبقاً . ' end
...
return "♡‿♡ أشترك بالقناة اولا ["..UserChaneel.."] \n♡‿♡ ثم تفعيل ارسل تفعيل ."
end
else
return "♡‿♡ أشترك بالقناة اولا ["..UserChaneel.."] \n♡‿♡ ثم ارجع ارسل تفعيل ."
end
end
...
return sendMsg(msg.chat_id_,msg.id_,'♡‿♡ *│*لا يمكنني تفعيل البوت في المجموعه‏ يجب أن يكون اكثر مـن *【'..GroupUsers..'】* عضـو .')
else 
GetChatMember(msg.chat_id_,our_id,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
return sendMsg(msg.chat_id_,msg.id_,'♡‿♡ **عذرا البوت ليس ادمن  في المجموعه\n♡‿♡ **يرجى ترقيته ادمن لتتمكن من تفعيل البوت .')
else
local lock_service = redis:get(speed..'lock_service')
if lock_service then 
sendMsg(msg.chat_id_,msg.id_,'♡‿♡ *تـم تـفـعـيـل الـمـجـمـوعـه .️ \n♡‿♡ *وتم رفع جمـيع المشرفين ادمن .‏‏‏ \n @SouRce_speeder')
else
sendMsg(msg.chat_id_,msg.id_,'♡‿♡ تـم تـفـعـيـل الـمـجـمـوعـه‏‏ \n')
end
GetChannelAdministrators(msg.chat_id_,function(arg,data)
for k,v in pairs(data.members_) do
...
USERNAME_T = '♡‿♡ **الـمعرف ❪ @['..datai.username_..'] ❫\n'
else 
USERNAME_T = ''
end
return send_msg(SUDO_ID,'♡‿♡ ** تــــم تــفــعــيــل الـــبـــوت ...\n———————————————\n♡‿♡ معلومات المجموعه\n'
..'♡‿♡ اسم المجموعه ❪ ['..NameGroup..']('..Gp_Link..') ❫\n'
..'♡‿♡ ايدي المجموعه ❪ '..msg.chat_id_..' ❫\n'
..'♡‿♡ عدد اعضاء المجموعه ❪ *'..Groupcount..'* ❫\n———————————————\n♡‿♡ معلومات الشخص الي ضافني \n'
..'♡‿♡ **الاسـم ❪ ['..FlterName(datai.first_name_..' '..(datai.last_name_ or ""),23)..'](tg://user?id='..msg.sender_user_id_..') ❫\n'
..USERNAME_T..'\n♡‿♡ تاريخ التفعيل ❪* '..os.date("%Y/%m/%d")
..' *❫\n♡‿♡ الساعه ❪* '..os.date("%I:%M%p")..' *❫')
end,nil)
end
end)
end,Get_Director) 
...
function modrem(msg)
if not msg.SudoUser then return '♡‿♡ ** أنـت لـسـت الـمـطـور .'end
if not redis:get(speed..'group:add'..msg.chat_id_) then return '** المجموعه تم تعطيلها مُسبقاً .' end  
rem_data_group(msg.chat_id_)
return '♡‿♡ ** تـم تـعـطـيـل الـمـجـمـوعـه .'
end

function modrem(msg)
if not msg.SudoUser then return '♡‿♡ **أنـت لـسـت الـمـطـور .'end
if not redis:get(speed..'group:add'..msg.chat_id_) then return '♡‿♡ **المجموعه تم تعطيلها مُسبقاً .' end  
rem_data_group(msg.chat_id_)
return '♡‿♡ **تـم تـعـطـيـل الـمـجـمـوعـه .'
end
...
if cmd =="tqeed" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد البوت\n") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المطور الاساسي\n") 
elseif redis:sismember(speed..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المطور\n") 
elseif redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المنشئ\n") 
elseif redis:sismember(speed..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المدير\n") 
elseif redis:sismember(speed..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد الادمن\n") 
end
GetChatMember(ChatID,UserID,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
Restrict(ChatID,UserID,1)
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم تقييد آلعضـو بنجآح \n',17,USERCAR) 
else
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنني تقييد المشرف\n") 
end
end)
end
...
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » '..namei..' \n♡‿♡ رسائلك » ❪ '..maseegs..' ❫ رسالةة\n♡‿♡ التفاعل »  ❪ '..Get_Ttl(maseegs)..' ❫\n',12,utf8.len(namei)) 
end

if cmd == "ban" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر البوت\n") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر المطور الاساسي\n") 
elseif UserID == 1317966118 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر مطور السورس\n") 
elseif UserID == 1424388256 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر مبرمج السورس\n") 
elseif UserID == 1463818802 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر مالك السورس\n") 
elseif redis:sismember(speed..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر المطور\n") 
elseif redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر المنشئ\n") 
elseif redis:sismember(speed..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر المدير\n") 
elseif redis:sismember(speed..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك حظر الادمن\n") 
end
kick_user(UserID,ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(ChatID,MsgID,'♡‿♡ **لا يمكنني حظر العضو .\n♡‿♡ **لانه مشرف في المجموعه \n .')    
elseif data.ID == "Error" and data.code_ == 3 then
return sendMsg(ChatID,MsgID,'♡‿♡ **لا يمكنني حظر العضو .\n♡‿♡ **ليس لدي صلاحيه الحظر او لست مشرف\n .')    
else
redis:hset(speed..'username:'..UserID, 'username', Resolv)
...
if cmd == "silent" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم البوت\n") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم المطور الاساسي\n♡‿♡ ") 
elseif UserID == 1317966118 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم مطور السورس\n") 
elseif UserID == 1424388256 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم مبرمج السورس\n") 
elseif UserID == 1463818802 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم مالك السورس\n") 
elseif redis:sismember(speed..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم المطور\n") 
elseif redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم المنشئ\n") 
elseif redis:sismember(speed..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم المدير\n") 
elseif redis:sismember(speed..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم الادمن\n") 
end
...
if GeneralBanned(UserID) then return sendMsg(ChatID,MsgID,'♡‿♡ ** العضو » ❪ '..USERNAME..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تم بالتأكيد حظره عام \n') end
redis:hset(speed..'username:'..UserID, 'username',Resolv)
...
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم بالتأكيد الغاء حظره العام \n',17,USERCAR) 
end
redis:srem(speed..'gban_users',UserID)
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم الغاء حظره العام \n',17,USERCAR) 
end

if cmd == "kick" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد البوت\n") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد المطور الاساسي\n") 
elseif UserID == 1317966118 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد مطور السورس\n") 
elseif UserID == 1424388256 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد مبرمج السورس\n") 
elseif UserID == 1463818802 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم مالك السورس\n") 
elseif redis:sismember(speed..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد المطور\n") 
elseif redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد المنشئ\n") 
elseif redis:sismember(speed..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد المدير\n") 
elseif redis:sismember(speed..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد الادمن\n") 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(ChatID,MsgID,'♡‿♡ ** لا يمكنني حظر العضو .\n♡‿♡ **لانه مشرف في المجموعه \n .')    
...
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم ترقيته ليصبح مطور \n',17,USERCAR) 
end

if cmd == "dn_sudo" then
if not redis:sismember(speed..':SUDO_BOT:',UserID) then 
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ انه بالتأكيد ليس مطور \n',17,USERCAR) 
end
redis:srem(speed..':SUDO_BOT:',UserID)
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم تنزيله من المطور \n',17,USERCAR) 
end  -- End Cmd 
end,nil)
else
return sendMsg(ChatID,MsgID,"♡‿♡ ** عذرا هذا العضو ليس موجود ضمن المجموعات \n .")
end 
end
...
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المطور الاساسي\n") 
elseif UserID == 1317966118 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد مطور السورس\n") 
elseif UserID == 1424388256 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد مبرمج السورس\n") 
elseif UserID == 1463818802 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد مالك السورس\n") 
elseif redis:sismember(speed..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المطور\n") 
elseif redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك تقييد المنشئ\n") 
...
if data.status_.ID == "ChatMemberStatusMember" then 
Restrict(ChatID,UserID,1)  
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تم تقييد آلعضـو بنجاح \n') 
else
return sendMsg(ChatID,MsgID,'♡‿♡ **لا يمكنني تقييد العضو .\n♡‿♡ **لانه مشرف في المجموعه \n .')    
end
end)
else
...
if data.status_.ID == "ChatMemberStatusEditor" then 
Restrict(ChatID,UserID,2)
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تم فك تقييد آلعضـو بنجآح \n') 
else
return sendMsg(ChatID,MsgID,'♡‿♡ **لا يمكنني قك تقييد العضو .\n♡‿♡ **لانني لست مشرف في المجموعه \n')    
end
end)
end,nil)
end
...
if redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **انه بالتأكيد منشىء في البوت \n') 
end
redis:hset(speed..'username:'..UserID, 'username', UserName)
redis:sadd(speed..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تمت ترقيته ليصبح منشى في البوت \n')
end

if cmd == "remmnsha" then
if not redis:sismember(speed..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **انه بالتأكيد ليس منشى \n') 
end
redis:srem(speed..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تمت تنزيله من المنشىء \n') 
end
...
if redis:sismember(speed..'whitelist:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **انه بالتأكيد من عضو مميز \n') 
end
redis:hset(speed..'username:'..UserID, 'username', UserName)
redis:sadd(speed..'whitelist:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تمت ترقيته ليصبح ضمن عضو مميز \n')
end

if cmd == "remwhitelist" then
if not redis:sismember(speed..'whitelist:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **انه بالتأكيد ليس من عضو مميز \n') 
end
redis:srem(speed..'whitelist:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تمت تنزيله من عضو مميز \n')
end
...
if cmd == "iduser" then
return sendMsg(ChatID,MsgID,"♡‿♡ ** أضغط على الايدي ليتم النسخ\n\n "..UserName.." ~⪼ ( `"..UserID.."` )")
end

if cmd == "whois" then
return sendMsg(ChatID,MsgID,
  '♡‿♡ ** الاسم » '..FlterName(data.title_,30)..'\n'
..'♡‿♡ ** الايدي » {`'..UserID..'`} \n'
..'♡‿♡ ** المعرف » '..UserName..'\n'
..'♡‿♡ ** نوع الكشف » بالمعرف\n'
..'')
...
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"♡‿♡ **عذرا لا يمكنني حظر نفسي \n") 
elseif data.type_.ID == "ChannelChatInfo" then 
return sendMsg(ChatID,MsgID,"♡‿♡ **عذرا لا يمكن حظر قناة \n") 
end
...
if cmd == "unban" then
if not redis:sismember(speed..'banned:'..ChatID,UserID) then 
sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تم بالتأكيد الغاء حظره \n') 
else
sendMsg(ChatID,MsgID,'♡‿♡ **العضو » ❪ '..UserName..' ❫\n♡‿♡ **الايدي » ❪ `'..UserID..'` ❫\n♡‿♡ **تم الغاء حظره \n')
end
redis:srem(speed..'banned:'..ChatID,UserID)
...
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد مبرمج السورس\n") 
elseif UserID == 1463818802 then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد مالك السورس\n") 
elseif redis:sismember(speed..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك طرد المطور\n") 
elseif
...
kick_user(UserID,ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
return sendMsg(ChatID,MsgID,'♡‿♡ ** لا يمكنني طرد العضو .\n♡‿♡ **لانه مشرف في المجموعه \n')    
elseif data.ID == "Error" and data.code_ == 3 then
return sendMsg(ChatID,MsgID,'♡‿♡ ** لا يمكنني طرد العضو .\n♡‿♡ **ليس لدي صلاحيه الحظر او لست مشرف\n')    
end
redis:hset(speed..'username:'..UserID, 'username', UserName)
...
return sendMsg(ChatID,MsgID,"♡‿♡ ** لا يوجد عضـو بهذا المعرف \n")
end 
...
sendMsg(ChatID,MsgID,"♡‿♡ ** العضو لا يوجد\n") 
return false
end
local UserID = data.id_
local Resolv = ResolveUser(data)
...
if cmd == "dn_sudo" then
if not redis:sismember(speed..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'♡‿♡ **العضو » '..USERNAME..' \n♡‿♡ ** الايدي » (`'..UserID..'`)\n♡‿♡ **انه بالتأكيد ليس مطور \n') end
redis:srem(speed..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم تنزيله من المطور \n',17,USERCAR) 
end
...
if not Check_Banned(ChatID,UserID) then 
 SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم بالتأكيد الغاء حظره \n',17,USERCAR) 
else
 SendMention(ChatID,UserID,MsgID,'♡‿♡ العضو » ❪ '..USERNAME..' ❫\n♡‿♡ الايدي » ❪ '..UserID..' ❫\n♡‿♡ تم الغاء حظره \n',17,USERCAR) 
end
redis:hset(speed..'username:'..UserID, 'username', Resolv)
...
if redis:sismember(speed..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"♡‿♡ **لا يمكنك كتم المدراء او الادمنيه\n") 
end
...
if redis:get(speed..'welcome:get'..msg.chat_id_) then welcomex = "❪ مفعل ❫" else welcomex = "❪ معطل ❫" end
if redis:get(speed..'lock_id'..msg.chat_id_) then lock_idx = "❪ مفعل ❫" else lock_idx = "❪ معطل ❫" end
if redis:get(speed..'replay'..msg.chat_id_)  then replayx  = "❪ مفعل ❫" else replayx  = "❪ معطل ❫" end
if redis:get(speed..'lock_woring'..msg.chat_id_) then lock_woringx = "❪ مفعل ❫" else lock_woringx  = "❪ معطل ❫" end
list_settings = "*♡‿♡*` اعدادات المجموعه :` \n"
.."\n♡‿♡ التعديل «» "..(redis:get(speed..'lock_edit'..msg.chat_id_) or 'false')
.."\n♡‿♡ الروابط «» "..(redis:get(speed..'lock_link'..msg.chat_id_) or 'false')
.."\n♡‿♡ التاك «» "..(redis:get(speed..'lock_tag'..msg.chat_id_) or 'false')
.."\n♡‿♡ المعرفات «» "..(redis:get(speed..'lock_username'..msg.chat_id_) or 'false')
.."\n♡‿♡ التكرار «» "..(redis:get(speed..'lock_flood'..msg.chat_id_) or 'false')
.."\n♡‿♡ الكلايش «» "..(redis:get(speed..'lock_spam'..msg.chat_id_) or 'false')
.."\n♡‿♡ الويب «» "..(redis:get(speed..'lock_webpage'..msg.chat_id_) or 'false')
.."\n♡‿♡ الماركدوان «» "..(redis:get(speed..'lock_markdown'..msg.chat_id_) or 'false')
.."\n♡‿♡ بالطرد البوتات «» "..(redis:get(speed..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n♡‿♡ البوتات «» "..(redis:get(speed..'lock_bots'..msg.chat_id_) or 'false')
.."\n♡‿♡ عدد التكرار «» "..(redis:get(speed..'num_msg_speed'..msg.chat_id_) or 'false')
.."\n\nـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ\n♡‿♡`اعدادات الوسائط :`\n"
.."\n*♡‿♡ *المتحركه «» "..(redis:get(speed..'mute_gif'..msg.chat_id_) or 'false')
.."\n♡‿♡ الدردشه «» "..(redis:get(speed..'mute_text'..msg.chat_id_) or 'false')
.."\n♡‿♡ الانلاين «» "..(redis:get(speed..'mute_inline'..msg.chat_id_) or 'false')
.."\n♡‿♡ الالعاب «» "..(redis:get(speed..'mute_game'..msg.chat_id_) or 'false')
.."\n♡‿♡ الصور «» "..(redis:get(speed..'mute_photo'..msg.chat_id_) or 'false')
.."\n♡‿♡ الفيديو «» "..(redis:get(speed..'mute_video'..msg.chat_id_) or 'false')
.."\n♡‿♡ البصمات «» "..(redis:get(speed..'mute_audio'..msg.chat_id_) or 'false')
.."\n♡‿♡ الصوت «» "..(redis:get(speed..'mute_voice'..msg.chat_id_) or 'false')
.."\n♡‿♡ الملصقات «» "..(redis:get(speed..'mute_sticker'..msg.chat_id_) or 'false')
.."\n♡‿♡ الجهات «» "..(redis:get(speed..'mute_contact'..msg.chat_id_) or 'false')
.."\n♡‿♡ التوجيه «» "..(redis:get(speed..'mute_forward'..msg.chat_id_) or 'false')
.."\n♡‿♡ الموقع «» "..(redis:get(speed..'mute_location'..msg.chat_id_) or 'false')
.."\n♡‿♡ الملفات «» "..(redis:get(speed..'mute_document'..msg.chat_id_) or 'false')
.."\n♡‿♡ الاشعارات «» "..(redis:get(speed..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n♡‿♡ الكيبورد «» "..(redis:get(speed..'mute_keyboard'..msg.chat_id_) or 'false')
.."\n\nـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ\n*♡‿♡*`اعدادات اخرى : `"
.."\n\n*♡‿♡*الترحيب «» "..welcomex
.."\n*♡‿♡*الردود «» "..replayx
.."\n*♡‿♡*التحذير «» "..lock_woringx
.."\n*♡‿♡*الايدي «» "..lock_idx
...
.."\n*♡‿♡*الويب «» "..(redis:get(speed..'lock_webpage'..msg.chat_id_) or 'false')
.."\n*♡‿♡*الماركدوان «» "..(redis:get(speed..'lock_markdown'..msg.chat_id_) or 'false')
.."\n*♡‿♡*البوتات بالطرد «» "..(redis:get(speed..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n*♡‿♡*البوتات «» "..(redis:get(speed..'lock_bots'..msg.chat_id_) or 'false')
.."\n*♡‿♡*عدد التكرار «» "..(redis:get(speed..'num_msg_speed'..msg.chat_id_) or 'false')
.."\nـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ\n*♡‿♡*اعدادات التقـييد :\n"
.."\n*♡‿♡*التقييد بالتوجيه «» "..(redis:get(speed..':tqeed_fwd:'..msg.chat_id_) or 'false')
.."\n*♡‿♡*التقييد بالصور «» "..(redis:get(speed..':tqeed_photo:'..msg.chat_id_) or 'false')
.."\n*♡‿♡*التقييد بالروابط «» "..(redis:get(speed..':tqeed_link:'..msg.chat_id_) or 'false')
.."\n*♡‿♡*التقييد بالمتحركه «» "..(redis:get(speed..':tqeed_gif:'..msg.chat_id_) or 'false')
.."\n*♡‿♡*التقييد الفيديو «» "..(redis:get(speed..':tqeed_video:'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '❬ مقفول ❭')
list_settings = list_settings:gsub('false', '❬ مفتوح ❭')
...
function media(msg)
list_settings = "*♡‿♡*`اعدادات الوسائط:`\n"
.."\n*♡‿♡*المتحركه «» "..(redis:get(speed..'mute_gif'..msg.chat_id_) or 'false')
.."\n*♡‿♡*الدردشه «» "..(redis:get(speed..'mute_text'..msg.chat_id_) or 'false')
.."\n*♡‿♡*الانلاين «» "..(redis:get(speed..'mute_inline'..msg.chat_id_) or 'false')
.."\n*♡‿♡*الالعاب «» "..(redis:get(speed..'mute_game'..msg.chat_id_) or 'false')
